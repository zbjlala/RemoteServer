...
namespace Server_Helper
{
    ...
    public class RedisHelper
    {
       ...
       // private static Log4netHelper log4NetHelper = new Log4netHelper();
        ...
        static RedisHelper()
        {
            var enviroment = Environment.GetEnvironmentVariable("ConfigType");
            
            if (redis == null || redis.IsConnected)
            {
                //redis = ConnectionMultiplexer.Connect("10.11.71.28:6379, abortConnect=false");
                //redis = ConnectionMultiplexer.Connect("aliyun-test-redis01.chanjet.com.cn:7009,password=Ufpark2015");
                if (enviroment == "moni" || enviroment == "prod")
                {
                    redis = ConnectionMultiplexer.Connect("d608291cc89e41cd.m.cnbja.kvstore.aliyuncs.com:6379,password=Yonyou2015");
                }
                else
                {
                    redis = ConnectionMultiplexer.Connect("aliyun-test-redis01.chanjet.com.cn:7009,password=Ufpark2015");
                ...
                {
                    ConsoleHelper.WriteLine("Redis连接成功...", ConsoleColor.Green);
                  //  Log4netHelper.WriteMessage("Redis 连接成功", System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.FullName.ToString(), LogLevel.Info);
                }
                else
                {
                    ConsoleHelper.WriteLine("Rdis 连接失败...", ConsoleColor.Green);
                   // Log4netHelper.WriteMessage("Redis 连接失败", System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.FullName.ToString());
                }
